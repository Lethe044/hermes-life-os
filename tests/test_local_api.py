...
def client(tmp_path, monkeypatch):
    ...
    for mod in ("storage", "life_score", "achievements", "recommendations", "leaderboard", "moon", "sleep_debt", "day_of_week", "habit_milestones", "goal_deadlines", "tools", "local_api"):
...
class TestMultiUser:
    ...
    def multi_user_client(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        for mod in ("storage", "users", "life_score", "achievements", "recommendations", "leaderboard", "moon", "sleep_debt", "day_of_week", "habit_milestones", "goal_deadlines", "tools", "local_api"):
            if mod in sys.modules:
                del sys.modules[mod]
        import local_api
        importlib.reload(local_api)
